    ...
    public void run() {
        if (clientMessage.getMessageType() == MessageType.UPDATE_CHORD) {
            ...
            if (key == AppConfig.myServentInfo.getChordId()) {
                ...
                AppConfig.chordState.addToVirtualStorage(fileInfo, additionInfoMsg.getSenderIpAddress(), additionInfoMsg.getSenderPort());
            }
            else {
            ...
            AppConfig.timestampedErrorPrint("Update chord handler got message that's not of type UPDATE_CHORD.");
        }
